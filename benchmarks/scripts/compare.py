#!/usr/bin/env python3
"""Benchmark comparison: light_server vs LitServe using wrk.
...
def plot_results(rows: list[dict], output_dir: Path) -> None:
    ...
    for w in workers_set:
        ...
        ax_rps.plot(concs, [r["ls_rps"] for r in subset], "o-", label=f"light_server w={w}")
        ax_rps.plot(concs, [r["lit_rps"] for r in subset], "s--", label=f"LitServe w={w}")

        ax_lat.plot(concs, [r["ls_p99"] for r in subset], "o-", label=f"light_server w={w}")
    ...
    if len(scale_rows) > 1:
        ...
        ax2.plot(ws, [r["ls_rps"] for r in scale_rows], "o-", label="light_server")
...
def _detect_model_name(model_repo: Path) -> str:
    """Detect model name from model_repo directory structure."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare light_server vs LitServe performance with wrk"
    )
    ...
    model_name = _detect_model_name(model_repo)

    lua_script = scripts_dir / "wrk_post.lua"
    output_path = (scripts_dir / args.output).resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)

    ls_url = f"http://127.0.0.1:8000/v2/models/{model_name}/infer"
    lit_url = f"http://127.0.0.1:8001/v2/models/{model_name}/infer"
    ls_script = scripts_dir / "run_lightserver.py"
    ...
    for workers in workers_list:
        for conc in concurrency_list:
            ...
            if not wait_for_server(ls_url, timeout=30):
                print("ERROR: light_server failed to start", file=sys.stderr)
                ls_proc.terminate()
                ls_proc.wait()
                continue

            time.sleep(args.warmup)
            print(f"[light_server] wrk -c{conc} -d{int(args.duration)}s ...")
            ls_res = run_wrk(args.wrk, ls_url, conc, args.duration, lua_script)
            ls_proc.terminate()
            ls_proc.wait()
            ...
            if not wait_for_server(lit_url, timeout=30):
            ...
            row = {
                "workers": workers,
                "concurrency": conc,
                "ls_rps": round(ls_res.get("rps", 0), 2),
                "ls_p99": round(ls_res.get("p99", 0), 3),
                "ls_p90": round(ls_res.get("p90", 0), 3),
                "ls_p50": round(ls_res.get("p50", 0), 3),
            ...
            speedup = (
                row["ls_rps"] / row["lit_rps"]
                if row["lit_rps"] > 0
                else float("inf")
            )
            print(f"  light_server: {row['ls_rps']:.1f} req/s  p99={row['ls_p99']:.2f}ms")
            print(f"  LitServe:     {row['lit_rps']:.1f} req/s  p99={row['lit_p99']:.2f}ms")
            print(f"  light_server / LitServe = {speedup:.2f}x")
