...
Requires litserve to be installed in the Python environment:
    uv pip install litserve    # or: pip install litserve
"""
...
def _load_model_from_repo(repo_path: Path):
    ...
    for model_dir in repo_path.iterdir():
        ...
        for version_dir in model_dir.iterdir():
            ...
            if model_py.exists():
                ...
                if cls:
                    instance = cls()
                    # Align api_path with light_server convention if still default
...
class _BuiltinSleepAPI(ls.LitAPI):
    ...
    def __init__(self):
        super().__init__(api_path="/v2/models/sleep_model/infer")
    ...
    def predict(self, inputs, **kwargs):
        ...
        if isinstance(inputs, list):
            return [{"output": i, "sleep_ms": 10} for i in inputs]
        return {"output": inputs, "sleep_ms": 10}
...
def main() -> int:
    ...
    parser.add_argument("--fast-queue", action="store_true", help="Use ZMQ transport instead of MPQueue")
    ...
    if args.model_repo:
        ...
        if not repo_path.exists():
            ...
            return 1
        loaded_name, api = _load_model_from_repo(repo_path)
        if api is None:
        ...
        print("Using built-in sleep model")
