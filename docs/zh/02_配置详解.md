...
server:
  ...
  log_level: "info"
  num_api_servers: 1
  http_workers: null       # null = 自动（max(1, cpu_count() - 1)）
  transport: "mp"          # "mp" 或 "zmq"
...
logging:
  ...
  level: "info"
  format: "json"
  output: null          # 未生效：服务器实际使用 info_output/error_output
  info_output: null
  error_output: null
  rotation: "daily"      # 未生效：服务器实际使用 rotate_by
  rotate_by: "none"
  max_size: 100
  when: "midnight"
  backup_count: 7
...
| `num_api_servers` | int | `1` | 已废弃：请使用 `http_workers` |
| `http_workers` | int | `null` | HTTP 工作进程数；`null` = 自动（`max(1, cpu_count() - 1)`） |
| `transport` | str | `"mp"` | 进程间传输方式：`"mp"`（多进程队列）或 `"zmq"`（ZeroMQ） |
...
3. **全局 `server.yaml` 的 `models` 字段** — 单模型覆盖配置
4. **全局 `server.yaml` 的 `server` 字段**（默认值）
