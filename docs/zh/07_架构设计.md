...
graph TB
    ...
    subgraph "HTTP 进程 (Uvicorn workers=1 或多进程)"
        FA[FastAPI App]
...
| HTTP 进程 | 1 或多个 | 处理 HTTP 请求。默认 `uvicorn workers=1` 以访问共享状态；设置 `http_workers` 可启用多进程模式 |
...
graph LR
    ...
    MR --> M2[model_b/]
    M1 --> MC1[model_config.yaml<br/>模型级策略]
    M1 --> V1[1/]
    M1 --> V2[2/]
    V1 --> F1[model.py<br/>LitAPI subclass]
    V1 --> F2[config.yaml<br/>版本配置]
    V2 --> F3[model.py]
    V2 --> F4[config.yaml]
...
| `Response Buffer` | `TTLResponseBuffer` | HTTP 进程内等待 worker 响应，带 TTL 过期和容量上限，防止内存泄漏 |
